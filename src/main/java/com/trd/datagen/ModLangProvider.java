...
// English
        ...
        // ═══ Энергосеть ═══
        add("tooltip.trd.machine.machine_battery.desc", "|Machine Battery| - a frame for |energy cells|, whose combined capacity defines the |JE energy| storage. Cells are inserted into 4 sockets on the front side (|RMB| with a cell) and removed with a |screwdriver|. The 16 item slots automatically charge and discharge items, finished items are moved to the output slots. The mode button switches the ports between: input and output, input only, output only, locked. The priority button defines the network fill order: high priority is filled first, and in case of deficit the network drains low priority batteries first.");
        add("tooltip.trd.machine.switch.desc", "|Switch| - a breaker of the |energy network|: it opens the network by excluding itself from it. |RMB| toggles the state, a |redstone signal| edge also flips the switch. Connects to wires and network participants with its side faces.");
        add("tooltip.trd.machine.converter.desc", "|Energy Converter| - converts the mod's |JE energy| into standard |Forge energy| and back. A |screwdriver| sets the mode: bi-directional, export only or import only, another click cycles the transfer limit. Actively pushes energy into adjacent storages if the mode allows it.");
        add("tooltip.trd.machine.paintable_wire.desc", "|Paintable Wire| - an |energy network| wire able to camouflage as other blocks. |RMB| with any block item repaints the wire into its look without consuming the item.");
        add("tooltip.trd.machine.energy_cell.desc", "|Energy Cell| - a replaceable storage for the |machine battery|. Inserted into the sockets on the front of the frame, removed with a |screwdriver| while keeping its charge. The capacity and speeds of the frame are summed from all inserted cells.");
        add("tooltip.trd.machine.battery.desc", "|Battery| - a portable |JE energy| storage. Charged and discharged in the slots of the |machine battery| or by the energy network. The charge level is shown by the bar on the item.");
        add("tooltip.trd.machine.creative_battery.desc", "|Creative Battery| - an inexhaustible |JE energy| source. Its charge never runs out.");

        // ═══ Кинетика ═══
        add("tooltip.trd.machine.hand_crank.desc", "|Hand Crank| - the simplest source of rotation. Cranked with the |mouse wheel| while looking at the block, each click sets the speed. Outputs torque only through its back side and connects exclusively to |light shafts|.");
        add("tooltip.trd.machine.bearing.desc", "|Bearing| - a pass-through node for a shaft through a block. |RMB| with a shaft inserts it inside, |Shift + RMB| with an empty hand extracts it. Transmits |torque| along its axis, the speed and torque limits are taken from the inserted shaft material.");
        add("tooltip.trd.machine.tachometer.desc", "|Tachometer| - a kinetic network sensor: shows the speed, total and consumed |torque|, inertia and network load. Inserted with a shaft like a |bearing|, extracted with |Shift + RMB| with an empty hand. Connects to shafts on both sides if the diameter matches.");
        add("tooltip.trd.machine.motor_electro.desc", "|Electric Motor| - a |torque| source powered by |JE energy|. Energy is accepted only from the back side, the target speed from 100 to 1000 RPM is set with a slider in the interface. Torque depends on the speed, and the motor stalls when energy runs out. A |redstone signal| edge switches the rotation direction. Connects only to |light shafts| on the side of its front.");
        add("tooltip.trd.machine.flywheel.desc", "|Flywheel| - a shaft attachment adding 500 inertia to the system. Smooths the network motion and provides a rotation reserve under varying load. |RMB| on a shaft attaches it, removed with a |screwdriver|.");
        add("tooltip.trd.machine.pulley.desc", "|Pulley| - a shaft attachment for belt transmission. |RMB| on a shaft attaches the pulley, two pulleys are connected with a |belt|. The transmission ratio depends on the difference in pulley diameters.");
        add("tooltip.trd.machine.gear.desc", "|Gear| - a shaft attachment for side torque transmission. Meshes with gears on adjacent shafts, inverting the rotation and setting the transmission ratio by size. A shaft with any gear conducts rotation to all 6 sides. Removed with a |screwdriver|.");
        add("tooltip.trd.machine.bevel_gear.desc", "|Bevel Gear| - an attachment for the shaft end to transmit |torque| at a right angle. Connects perpendicular shafts whose bevel points are close, with a 1 to 1 ratio. Removed with a |screwdriver|.");

        // ═══ Конвейеры ═══
        add("tooltip.trd.machine.conveyor.desc", "|Conveyor| - a belt for moving items. Items travel in the belt direction and at the end are passed to a |conveyor inserter| or the next line. Belts of the same direction merge into a single network.");
        add("tooltip.trd.machine.conveyor_inserter.desc", "|Conveyor Inserter| - a middleman between the belt and machines. Items from the end of the belt enter the inserter buffer and are moved into the inventory of the machine behind it. When the buffer overflows, excess items are ejected upwards.");
        add("tooltip.trd.machine.conveyor_extractor.desc", "|Conveyor Extractor| - a middleman between a machine and the belt. It pulls up to 3 items from the machine slots behind it into its buffer and feeds them onto the belt in front one by one.");
        add("tooltip.trd.machine.sortirovshik.desc", "|Conveyor Sorter| - a filter for items traveling along a belt next to it. It is not part of the conveyor chain itself: it scans the segment of the adjacent belt and redirects items that pass the filter towards the matching colored side. Each of the 6 sections has its own color matching a face: red is always on top, magenta on the bottom, the rest are on the sides. Sections operate in 4 modes: closed direction, blacklist, whitelist and universal direction, which accepts items that matched no other section. Ghost items in the slots are remembered together with their NBT and are not consumed, clicking with an empty hand forgets the item. Output priority: an adjacent conveyor, a machine inventory, or ejection into the world.");

        // ═══ Промышленные машины ═══
        add("tooltip.trd.machine.electro_furnace.desc", "|Electric Furnace| - a fuel-free smelting furnace powered by |JE energy|. Energy is accepted only from the back side, consumption is 100 JE per second. Smelts the same recipes as a vanilla furnace, but 30% faster, and accumulates experience. Slots: input, output and a battery slot; automation from the bottom extracts the output, from the sides items are sorted into slots automatically.");
        add("tooltip.trd.machine.stanok.desc", "|Stanok| - a universal kinetic multi-block machine for material processing. Requires |torque| supplied through the kinetic ports on the sides of the controller. The tool head in the dedicated slot defines the operation type: the |press| stamps plates, the |wire carriage| rolls plates into wire coils, the |milling cutter| turns light titanium shafts. Each recipe requires its own RPM, work stops if the speed deviates by more than 25%. Controlled via the interface opened with |RMB| on the controller.");
        add("tooltip.trd.machine.coccer_oven.desc", "|Coccer Oven| - a multi-block retort for high-temperature processing. Heated by a |heater| from below, once the required temperature is reached it processes items dropped on top: wet sponge into dry sponge, cinnabar into |mercury|, glowstone into |sulfur|. Fluids exit through the corner ports, pouring fluids in from outside is forbidden. The processing speed grows as the temperature exceeds the required one. Careful: the oven ignites everything standing on it.");
        add("tooltip.trd.machine.vishelashivatel.desc", "|Leacher| - a kinetic machine for leaching valuable substances out of items with fluids. |Torque| is accepted only through the top side, fluids are poured in through the ports. The working fluid type is set with the |fluid identifier| in a dedicated slot and is remembered. For example, it processes rotten flesh into leather using hydrogen peroxide.");
        add("tooltip.trd.machine.centrifuge.desc", "|Centrifuge| - an electric machine for separating materials into fractions. Assembled from a |centrifuge motor| with the cone placed strictly on top of it. Consumes 250 JE per second from its internal buffer, energy is accepted from the bottom or the back of the motor. Separates dirt, gravel and bone blocks into several random output items.");
        add("tooltip.trd.machine.steel_storage.desc", "|Steel Storage| - a compact multi-block warehouse with 91 slots. Does not accept shulker boxes or other steel storages. When broken it drops as an item keeping all contents, its tooltip shows the fill level and the first item types.");
        add("tooltip.trd.machine.casting_pot.desc", "|Casting Pot| - part of the casting chain, holds the melt and a mold. The mold is inserted with an item matching a mold recipe, and extracted with a |screwdriver| or a hand. After collecting enough metal the pot creates a red-hot casting that cannot be picked up by hand until it cools down. After a long idle the metal turns into |slag|.");
        add("tooltip.trd.machine.casting_descent.desc", "|Casting Descent| - a trough that transfers the melt from the |smelter| behind it into the |casting pots| below. Transfers 3 metal units at a time, showing a stream while pouring.");

        // ═══ Жидкости ═══
        add("tooltip.trd.machine.fuel_tank.desc", "|Fuel Tank| - a multi-block fluid container immune to corrosion and leaks. The small tank holds 288000 mB, the large one is assembled as a 7x3x3 multi-block and holds 768000 mB. Filled through fluid connectors or buckets in the interface, the fluid filter is set with the |fluid identifier|. Keeps the fluid when dismantled, transfer between tanks is not limited in speed.");
        add("tooltip.trd.machine.fluid_barrel.desc", "|Fluid Barrel| - a basic fluid container: iron holds 16000 mB, lead 20000, steel 24000. Barrels are sensitive to their contents: overly hot fluid melts the barrel, corrosive fluid turns it into a leak or a rotten barrel. A |protector| in the dedicated slot raises heat and corrosion resistance. The button in the interface switches the mode: input and output, input only, output only, disabled. Damaged barrels keep losing fluid over time.");
        add("tooltip.trd.machine.valve.desc", "|Valve| - a shutoff element of the |fluid system|. |RMB| with an empty hand opens and closes the flow, a |redstone signal| controls it directly. The passing fluid filter is set with the |fluid identifier|.");
        add("tooltip.trd.machine.protector.desc", "|Protector| - a replaceable shield for a |fluid barrel|, inserted into the protector slot. Raises the barrel resistance to heat and corrosion: steel gives the biggest heat bonus, tungsten the best corrosion protection, lead is balanced.");
        add("tooltip.trd.machine.infinite_barrel.desc", "|Infinite Barrel| - a creative fluid source. Inserted into the output slot of a |fluid barrel|, it endlessly fills the barrel with the fluid set by its filter.");

        // ═══ Инструменты ═══
        add("tooltip.trd.machine.screwdriver.desc", "|Screwdriver| - a tuning tool for mechanisms. Configures the |energy converter|, removes attachments from shafts, coils from the |stator|, |energy cells| from the machine battery and the mold from the |casting pot|. Also regulates the heat output of the |chemical plant heater|.");
        add("tooltip.trd.machine.hammer.desc", "|Hammer| - a tool for smithing on the anvil. Used to strike the workpiece during manual forging.");
        add("tooltip.trd.machine.poker.desc", "|Poker| - a tool for handling hot metal. Allows taking a red-hot casting or slag from the |casting pot| without waiting for it to cool down. |Shift + RMB| dumps the entire pot contents into slag, the mold stays. Also extracts items from the |smelter|.");
        add("tooltip.trd.machine.detonator.desc", "|Detonator| - a remote trigger for charges. |Shift + RMB| on a block memorizes its position, |RMB| in the air detonates the |detonatable charge| placed there.");
        ...
        add("tooltip.trd.machine.range_detonator.desc", "|Long-Range Detonator| - an instant trigger with a straight beam. |RMB| fires a beam up to 256 blocks and detonates the |detonatable charge| under the crosshair. Does not memorize positions.");
        add("tooltip.trd.machine.cast_pickaxe_iron.desc", "|Cast Iron Pickaxe| - a heavy pickaxe with a charged strike. Requires an empty offhand, holding |RMB| charges a heavy swing that boosts damage and block breaking. At full charge it tears out up to 3 ores of one vein. Capable of splitting the |conglomerate|.");
        add("tooltip.trd.machine.cast_pickaxe_steel.desc", "|Cast Steel Pickaxe| - a heavy pickaxe with a charged strike and a diamond mining level. Holding |RMB| charges a swing, a full-power strike tunnels through 5 blocks in the look direction with fading force. Capable of splitting the |conglomerate|.");

        // ═══ Оружие ═══
        add("tooltip.trd.machine.turret_portative.desc", "|Portable Light Turret| - a one-time deployable 'Nagual' with an autonomous ammo supply. Placed with |RMB| on a surface, serves for 3 minutes or until 250 rounds are spent.");
        add("tooltip.trd.machine.pig_turret.desc", "|Pig Turret| - a mobile version of the light turret: a pig with a turret mounted on top. Placed with |RMB|, the pig is eternal, the turret fires on the move. Consumes the item.");
        add("tooltip.trd.machine.goggles.desc", "|Grenadier Goggles| - a protective mask reducing explosion damage taken by 30%. Part of the grenadier armor set.");
    ...
    private void addRussian() {
        ...
        // ═══ Энергосеть ═══
        add("tooltip.trd.machine.machine_battery.desc", "|Машинное энергохранилище| - каркас под |энергоячейки|, их суммарная ёмкость определяет запас |JE энергии|. Ячейки вставляются в 4 гнезда на лицевой стороне (|ПКМ| ячейкой), извлекаются |отвёрткой|. 16 слотов интерфейса автоматически заряжают и разряжают предметы, готовые перекладываются в слоты выдачи. Кнопка режима переключает работу портов: приём и передача, только приём, только передача, блокировка. Кнопка приоритета задаёт порядок наполнения сетью: высокий заполняется первым, а при нехватке энергии сеть забирает её в первую очередь из низкого приоритета.");
        add("tooltip.trd.machine.switch.desc", "|Рубильник| - выключатель |энергосети|: размыкает сеть, исключая себя из неё. |ПКМ| переключает состояние, фронт |редстоун-сигнала| тоже управляет рубильником. Соединяется с проводами и участниками сети боковыми гранями.");
        add("tooltip.trd.machine.converter.desc", "|Конвертер энергии| - преобразует |JE энергию| мода в стандартную |Forge энергию| и обратно. |Отвёрткой| задаётся режим: двунаправленный, только экспорт или только импорт, повторный клик меняет лимит передачи. Активно выталкивает энергию в соседние хранилища, если режим позволяет.");
        add("tooltip.trd.machine.paintable_wire.desc", "|Окрашиваемый провод| - провод |энергосети|, умеющий маскироваться под другие блоки. |ПКМ| любым блочным предметом перекрашивает провод в его вид, не расходуя предмет.");
        add("tooltip.trd.machine.energy_cell.desc", "|Энергоячейка| - сменный накопитель для |машинного энергохранилища|. Вставляется в гнёзда на лицевой стороне каркаса, извлекается |отвёрткой|, сохраняя заряд в себе. Ёмкость и скорости каркаса складываются из всех вставленных ячеек.");
        add("tooltip.trd.machine.battery.desc", "|Батарея| - портативный накопитель |JE энергии|. Заряжается и разряжается в слотах |машинного энергохранилища| или энергосетью. Уровень заряда показан полоской на предмете.");
        add("tooltip.trd.machine.creative_battery.desc", "|Креативная батарея| - неиссякаемый источник |JE энергии|. Заряд никогда не кончается.");

        // ═══ Кинетика ═══
        add("tooltip.trd.machine.hand_crank.desc", "|Ручная рукоятка| - простейший источник вращения. Крутится прокруткой |колеса мыши| при взгляде на блок, каждый щелчок задаёт обороты. Отдаёт момент только через тыльную сторону и соединяется исключительно с |лёгкими валами|.");
        add("tooltip.trd.machine.bearing.desc", "|Подшипник| - проходной узел для вала сквозь блок. |ПКМ| валом вставляет его внутрь, |Shift + ПКМ| пустой рукой извлекает. Передаёт |крутящий момент| вдоль своей оси, лимиты скорости и момента берутся из материала вставленного вала.");
        add("tooltip.trd.machine.tachometer.desc", "|Тахометр| - датчик кинетической сети: показывает обороты, общий и потреблённый |момент|, инерцию и нагрузку сети. Вставляется валом как |подшипник|, извлекается |Shift + ПКМ| пустой рукой. Соединяется с валами с обеих сторон при совпадении диаметра.");
        add("tooltip.trd.machine.motor_electro.desc", "|Электромотор| - источник |крутящего момента|, работающий от |JE энергии|. Энергия принимается только с задней стороны, целевые обороты от 100 до 1000 RPM задаются ползунком в интерфейсе. Момент зависит от оборотов, при нехватке энергии мотор останавливается. Фронт |редстоун-сигнала| переключает направление вращения. Соединяется только с |лёгкими валами| по направлению лицевой стороны.");
        add("tooltip.trd.machine.flywheel.desc", "|Маховик| - насадка на вал, добавляющая системе 500 инерции. Сглаживает ход сети и даёт запас вращения при переменной нагрузке. |ПКМ| по валу прикрепляет, снимается |отвёрткой|.");
        add("tooltip.trd.machine.pulley.desc", "|Шкив| - насадка на вал для ременной передачи. |ПКМ| по валу прикрепляет шкив, два шкива соединяются |ремнём|. Передаточное отношение зависит от разницы диаметров шкивов.");
        add("tooltip.trd.machine.gear.desc", "|Шестерня| - насадка на вал для боковой передачи момента. Зацепляется с шестернями соседних валов, меняя направление вращения и задавая передаточное отношение по размеру. Вал с любой шестернёй проводит вращение во все 6 сторон. Снимается |отвёрткой|.");
        add("tooltip.trd.machine.bevel_gear.desc", "|Коническая шестерня| - насадка на торец вала для передачи момента под прямым углом. Соединяет перпендикулярные валы, чьи конические точки находятся рядом, с отношением 1 к 1. Снимается |отвёрткой|.");

        // ═══ Конвейеры ═══
        add("tooltip.trd.machine.conveyor.desc", "|Конвейер| - лента для перемещения предметов. Предметы движутся по направлению ленты, а в конце передаются |конвейерному вставщику| или следующей линии. Ленты одного направления объединяются в единую сеть.");
        add("tooltip.trd.machine.conveyor_inserter.desc", "|Конвейерный вставщик| - посредник между лентой и машинами. Предметы с конца ленты попадают в буфер вставщика и перекладываются в инвентарь машины с тыльной стороны. При переполнении буфера излишки выбрасываются вверх.");
        add("tooltip.trd.machine.conveyor_extractor.desc", "|Конвейерный извлекатель| - посредник между машиной и лентой. Забирает из машины с тыльной стороны до 3 предметов за раз в свой буфер и выдаёт их на ленту спереди.");
        add("tooltip.trd.machine.sortirovshik.desc", "|Конвейерный сортировщик| - фильтр предметов, движущихся по ленте рядом с ним. Сам не является частью конвейерной цепи: сканирует сегмент соседней ленты и перенаправляет прошедшие фильтр предметы в сторону соответствующей цветной грани. Каждая из 6 секций имеет свой цвет грани: красная всегда сверху, маджента снизу, остальные по бокам. Секции работают в 4 режимах: направление закрыто, чёрный список, белый список и универсальное направление, принимающее предметы, не подошедшие ни одной другой секции. Фантомные предметы в слотах запоминаются вместе с NBT и не тратятся, клик пустой рукой забывает предмет. Порядок выдачи: соседний конвейер, инвентарь машины или выброс в мир.");

        // ═══ Промышленные машины ═══
        add("tooltip.trd.machine.electro_furnace.desc", "|Электропечь| - плавильная печь без топлива, работающая от |JE энергии|. Энергия принимается только с задней стороны, расход - 100 JE в секунду. Плавит те же рецепты, что и обычная печь, но на 30% быстрее и накапливает опыт. Слоты: вход, выход и батарея, автоматизация снизу забирает готовый выход, с боков предметы сами распределяются по слотам.");
        add("tooltip.trd.machine.stanok.desc", "|Станок| - универсальная кинетическая машина-мультиблок для обработки материалов. Требует подвода |крутящего момента| через кинетические порты по бокам контроллера. Насадка в специальном слоте определяет тип операций: |пресс| штампует пластины, |каретка-катушка| прокатывает пластины в проволочные катушки, |фреза| вытачивает лёгкие титановые валы. Каждый рецепт требует своих оборотов, при отклонении более чем на 25% работа останавливается. Управление - интерфейс, открывается |ПКМ| по контроллеру.");
        add("tooltip.trd.machine.coccer_oven.desc", "|Коксовая печь| - реторта-мультиблок для высокотемпературной переработки. Греется |нагревателем| снизу, при достижении нужной температуры перерабатывает предметы, подбрасываемые сверху: мокрую губку в сухую, киноварь в |ртуть|, светокамень в |серу|. Жидкости выходят через угловые порты, заливка жидкости извне запрещена. Скорость переработки растёт с превышением требуемой температуры. Осторожно: печь поджигает всё, что стоит на ней.");
        add("tooltip.trd.machine.vishelashivatel.desc", "|Выщелачиватель| - кинетическая машина для вымывания полезных веществ из предметов жидкостями. |Крутящий момент| принимается только через верхнюю грань, жидкость заливается через порты. Тип рабочей жидкости задаётся |идентификатором жидкостей| в отдельном слоте и запоминается. Например, перерабатывает гнилую плоть в кожу с расходом перекиси водорода.");
        add("tooltip.trd.machine.centrifuge.desc", "|Центрифуга| - электрическая машина для разделения материалов на фракции. Собирается из |мотора центрифуги| и конуса, устанавливаемого строго на него. Расходует 250 JE в секунду из внутреннего буфера, энергия принимается снизу или с тыла мотора. Разделяет землю, гравий и костный блок на несколько случайных выходных предметов.");
        add("tooltip.trd.machine.steel_storage.desc", "|Стальное хранилище| - компактный склад-мультиблок на 91 слот. Не принимает шалкеровые ящики и другие стальные хранилища. При разборке выпадает предметом со всем содержимым, тултип показывает заполненность и первые типы предметов.");
        add("tooltip.trd.machine.casting_pot.desc", "|Литейный котёл| - часть цепочки литья, хранит расплав и форму. Форма вставляется предметом с подходящим рецептом формы, извлекается |отвёрткой| или рукой. Набрав нужное количество металла, котёл создаёт раскалённую отливку, которую нельзя брать рукой до остывания. При долгом простое металл превращается в |шлак|.");
        add("tooltip.trd.machine.casting_descent.desc", "|Литейный спуск| - жёлоб, передающий расплав из |плавильни| позади себя в |литейные котлы| под собой. За раз передаёт 3 единицы металла, показывая струю при переливе.");

        // ═══ Жидкости ═══
        add("tooltip.trd.machine.fuel_tank.desc", "|Топливная цистерна| - многоблочная ёмкость для жидкостей, не подверженная коррозии и утечкам. Малая вмещает 288000 мБ, большая собирается мультиблоком 7x3x3 и вмещает 768000 мБ. Заполняется через жидкостные коннекторы или вёдра в интерфейсе, фильтр жидкости задаётся |идентификатором жидкостей|. Хранит жидкость при разборке, скорость перекачки между ёмкостями не ограничена.");
        add("tooltip.trd.machine.fluid_barrel.desc", "|Жидкостная бочка| - базовая ёмкость для жидкостей: железная вмещает 16000 мБ, свинцовая 20000, стальная 24000. Бочки чувствительны к содержимому: слишком горячая жидкость расплавит бочку, коррозионная превратит её в течь или прогнившую. |Протектор| в специальном слоте повышает стойкость к жару и коррозии. Кнопка в интерфейсе переключает режим: приём и выдача, только приём, только выдача, выключено. Повреждённые бочки теряют жидкость со временем.");
        add("tooltip.trd.machine.valve.desc", "|Клапан| - запорный элемент |жидкостной сети|. |ПКМ| пустой рукой открывает и закрывает поток, фронт |редстоун-сигнала| управляет им напрямую. Фильтр пропускаемой жидкости задаётся |идентификатором жидкостей|.");
        add("tooltip.trd.machine.protector.desc", "|Протектор| - сменная защита для |жидкостной бочки|, вставляется в слот протектора. Повышает стойкость бочки к жару и коррозии: стальной даёт наибольший прирост к жаростойкости, вольфрамовый лучшую защиту от коррозии, свинцовый сбалансирован.");
        add("tooltip.trd.machine.infinite_barrel.desc", "|Бесконечная бочка| - творческий источник жидкости. Вставляется в слот выдачи |жидкостной бочки| и бесконечно наполняет её жидкостью, заданной фильтром.");

        // ═══ Инструменты ═══
        add("tooltip.trd.machine.screwdriver.desc", "|Отвёртка| - настроечный инструмент механизмов. Настраивает |конвертер энергии|, снимает насадки с валов, катушки из |статора|, |энергоячейки| из энергохранилища и форму из |литейного котла|. Также регулирует выработку тепла |нагревателем химического завода|.");
        add("tooltip.trd.machine.hammer.desc", "|Молот| - инструмент для ковки на наковальне. Используется для ударов по заготовке при ручной ковке.");
        add("tooltip.trd.machine.poker.desc", "|Кочерга| - инструмент для работы с горячим металлом. Позволяет достать из |литейного котла| раскалённую отливку или шлак, не дожидаясь остывания. |Shift + ПКМ| сбрасывает всё содержимое котла в шлак, форма остаётся. Также достаёт предметы из |плавильни|.");
        add("tooltip.trd.machine.detonator.desc", "|Детонатор| - радиопульт для подрыва зарядов. |Shift + ПКМ| по блоку запоминает его позицию, |ПКМ| в воздухе детонирует установленный там |детонируемый заряд|.");
        ...
        add("tooltip.trd.machine.range_detonator.desc", "|Дальнобойный детонатор| - пульт мгновенного подрыва по прямой. |ПКМ| пускает луч до 256 блоков и детонирует |детонируемый заряд| под прицелом. Позиции не запоминает.");
        add("tooltip.trd.machine.cast_pickaxe_iron.desc", "|Литая железная кирка| - тяжёлая кирка с зарядом удара. Требует пустой второй руки, удержание |ПКМ| взводит тяжёлый замах, усиливающий урон и дробление блоков. На полном заряде вырывает до 3 руд одной жилы. Способна расколоть |конгломерат|.");
        add("tooltip.trd.machine.cast_pickaxe_steel.desc", "|Литая стальная кирка| - тяжёлая кирка с зарядом удара и алмазным уровнем добычи. Удержание |ПКМ| взводит замах, удар на полном заряде пробивает туннель из 5 блоков по взгляду с затуханием силы. Способна расколоть |конгломерат|.");

        // ═══ Оружие ═══
        add("tooltip.trd.machine.turret_portative.desc", "|Портативная лёгкая турель| - одноразовый размещаемый 'Нагуаль' с автономным боезапасом. Ставится |ПКМ| по поверхности и служит 3 минуты или до израсходования 250 патронов.");
        add("tooltip.trd.machine.pig_turret.desc", "|Турель на свинье| - мобильная лёгкая турель: свинья с установленной сверху турелью. Ставится |ПКМ|, свинья бессмертна, турель ведёт огонь на ходу. Расходует предмет.");
        add("tooltip.trd.machine.goggles.desc", "|Очки гренадёра| - защитная маска, снижающая получаемый урон от взрывов на 30%. Входит в комплект брони гренадёра.");
