...
public class ModLangProvider extends LanguageProvider {
    ...
    private void addEnglish() {
        ...
        // ═══ GUI: Optic Microscope ═══
        add("block.trd.optic_microscope", "Optic Microscope");
        add("gui.trd.optic_microscope", "Optic Microscope");
...
// English
 ...
 add("tooltip.trd.conglomerate.empty", "§7Empty chunk");
        ...
        add(ModBlocks.WIRE_FENCE_ALT.get(), "Mesh Fence");
        ...
        // ═══ Энергосеть ═══
        add("tooltip.trd.machine.machine_battery.desc", "|Modular Energy Storage| - a housing for |energy cells|, their combined capacity defines the maximum |JE energy| storage. Cells are inserted into the 4 sockets on the front side (|RMB| with a cell) and removed with a |screwdriver|.");
        add("tooltip.trd.machine.switch.desc", "|Switch| - a special mechanism capable of closing and opening the |energy network| by pressing |RMB| or when supplied with a redstone signal.");
        add("tooltip.trd.machine.converter.desc", "|Energy Converter| - converts |JE energy| into standard |Forge energy| and back. The |JE energy| throughput can be adjusted with a |Screwdriver|.");
        add("tooltip.trd.machine.paintable_wire.desc", "|Paintable Wire| - an |energy network| wire able to camouflage as other blocks. Pressing |RMB| with any block repaints the wire into its look without consuming the block itself.");
        add("tooltip.trd.machine.energy_cell.desc", "|Energy Cell| - a removable storage of the |modular energy storage|. Inserted into the sockets on the front side of the housing, removed with a |screwdriver| while keeping its charge inside. The capacity and energy transfer speed are summed from the values of all inserted cells.");
        ...
        add("tooltip.trd.machine.gear.desc", "|Gear| - a shaft attachment for side |torque| transmission. Meshes with gears on adjacent shafts, changing the rotation direction and setting the transmission ratio. Removed with a |screwdriver|.");
        ...
        // ═══ Промышленные машины ═══
        add("tooltip.trd.machine.electro_furnace.desc", "|Electric Furnace| - a furnace powered by |JE energy|. Energy is accepted from the back side, consumption is 100 JE per second. Smelts the same recipes as a vanilla furnace, but 30% faster, while accumulating experience.");
        add("tooltip.trd.machine.stanok.desc", "|Multi-Blanking Machine| - a universal kinetic machine for material processing. Requires continuous |torque| supply through the kinetic ports on the sides to operate. The tool head in the dedicated slot defines the operation type. Each recipe requires its own RPM, work stops if the speed deviates by more than 25%.");
        add("tooltip.trd.machine.coccer_oven.desc", "|Coke Oven| - a special furnace for high-temperature processing. The processing speed grows as the temperature exceeds the required one. It is not recommended to climb into the oven while it is running.");
        add("tooltip.trd.machine.vishelashivatel.desc", "|Ore Leacher| - a kinetic machine for processing items through interaction with fluids. The working fluid type is set with the |fluid identifier| in a dedicated slot.");
        add("tooltip.trd.machine.centrifuge.desc", "|Centrifuge| - an electric machine for separating materials into fractions. Assembled from a |centrifuge motor| and |special attachments|.");
        add("tooltip.trd.machine.steel_storage.desc", "|Steel Storage| - a portable multi-block warehouse with 91 slots. Does not accept shulkers and other portable storages.");
        add("tooltip.trd.machine.casting_pot.desc", "|Casting Pot| - part of the casting chain, required for casting parts through |casting molds|. After collecting the required amount of liquid metal, the pot creates a red-hot workpiece that cannot be picked up by hand until it fully cools down. After a long idle the metal turns into |slag|.");
        add("tooltip.trd.machine.casting_descent.desc", "|Casting Trough| - part of the casting chain, transferring liquid metal from the |smelter| behind it into the |casting pots|.");

        // ═══ Жидкости ═══
        add("tooltip.trd.machine.fuel_tank.desc", "|Tank| - a multi-block fluid container immune to corrosion and heat. The fluid type is set with the |fluid identifier|. Keeps the fluid when mined.");
        add("tooltip.trd.machine.fluid_barrel.desc", "|Fluid Barrel| - a basic fluid container. Barrels are sensitive to their contents: overly hot fluid will melt the barrel, and corrosive fluid will turn it into a sieve. A |protector| in the dedicated slot raises heat and corrosion resistance.");
        add("tooltip.trd.machine.valve.desc", "|Fluid Valve| - an element of the |fluid system| capable of stopping fluid transfer by pressing |RMB| or when supplied with a |redstone signal|. The passing fluid type is set with the |Fluid Identifier|.");
        add("tooltip.trd.machine.protector.desc", "|Protector| - a replaceable shield for a |fluid barrel|, inserted into the corresponding slot. Raises the barrel resistance to heat and corrosion.");

        // ═══ Инструменты ═══
        add("tooltip.trd.machine.screwdriver.desc", "|Screwdriver| - truly the most universal tool in our mod. Allows configuring plenty of machines and installations. (P.S. If you found some bug, before writing to us about it, try poking around with the screwdriver, who knows, everything might start working).");
        add("tooltip.trd.machine.hammer.desc", "|Hammer| - a special tool for smithing on the anvil. Used to strike the workpiece during manual forging.");
        add("tooltip.trd.machine.poker.desc", "|Poker| - a special tool for handling hot metal. Allows taking a red-hot workpiece or |slag| out of the |casting pot| without waiting for it to cool down. Pressing |Shift + RMB| on the |casting pot| dumps all of its contents into |slag|.");
        add("tooltip.trd.machine.detonator.desc", "|Detonator| - a radio remote for detonating charges. Pressing |Shift + RMB| on a block memorizes its position, pressing |RMB| in the air detonates the charge.");
    ...
    private void addRussian() {
        ...
        // ═══ GUI: Optic Microscope ═══
        add("block.trd.optic_microscope", "Оптический микроскоп");
        add("gui.trd.optic_microscope", "Оптический микроскоп");
...
// ═══ ТУЛТИПЫ АККУМУЛЯТОРОВ (ModBatteryItem) ═══
        ...
        add(ModBlocks.WIRE_FENCE_ALT.get(), "Сеточный забор");
