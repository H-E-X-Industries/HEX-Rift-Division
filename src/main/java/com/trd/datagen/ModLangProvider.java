...
// English
    ...
    private void addRussian() {
        ...
        add("tooltip.trd.machine.drobitel.desc", "|Рудный дробитель| - механическая установка, позволяющая проводить механическую обработку предметов и блоков. Способен принимать предметы через отверстие сверху (не рекомендуется залезать туда). Для работы требует специальные |лезвия| и непрерывную подачу |крутящего момента| через кинетические порты. Вставлять |лезвия| в дробитель нужно в количестве двух штук нажатием правой кнопкой мыши по корпусу. |Дробитель| способен обрабатывать все 9 слотов на входе за раз, но потребление |крутящего момента| будет расти за каждый новый задействованный слот. При превышении максимальной скорости |дробитель| сломается.");
        add("tooltip.trd.machine.blade.desc", "|Лезвия рудного дробителя| - обязательный компонент к |рудному дробителю|, позволяющий ему функционировать. |Лезвия| из разных материалов имеют разные диапазоны рабочих скоростей, необходимых для их оптимальной работы. Вне диапазона рабочей скорости, |лезвия| будут работать со штрафом к |крутящему моменту|.");
        add("tooltip.trd.machine.steam_engine.desc", "|Паровой двигатель| - механизм, позволяющий генерировать |крутящий момент| благодаря работе парового поршня. Вводить пар необходимо через верхние жидкостные входы, а отработанный пар низкого давления через нижние жидкостные выходы. Мощности нескольких соединенных друг за другом |паровых двигателей| суммируются.");
        ...
        add("tooltip.trd.machine.water_pump.desc", "|Жидкостная помпа| - механизм, предназначенный для выкачки жидкостей в больших масштабах. Для работы требует непризывную подачу |крутящего момента| через кинетические порты. Чем больше жидкостный бассейн, тем эффективнее выкачка.");
        ...
        add("tooltip.trd.machine.turret_light.desc", "|Легкая десантная турель 'Нагваль'| - |Автоматический вид вооружения|, позволяющий контролировать местность. Для работы требует |20-мм патроны турельного типа| и |JE энергию|. Обладает высокотехнологичным меню, в котором можно настроить режим работы, просмотреть статистику и управлять белым списком игроков (необходимо установить |турельный боевой чип|). Для визуализации процессов наводки, нажмите клавишу F3.");
        ...
        add("tooltip.trd.machine.fluid_identifier.desc", "|Жидкостный идентификатор| - специальный инструмент, способный задавать элементам |жидкостной системы| тип жидкости. |ПКМ| назначает тип жидкости. |Shift + ПКМ| задаёт ряду соединённых элементов тип жидкости. |Shift + ПКМ| по воздуху открывает интерфейс инструмента.");

        // ═══ Энергосеть ═══
        add("tooltip.trd.machine.machine_battery.desc", "|Модульное энергохранилище| - корпус для |энергоячеек|, их суммарная ёмкость определяет максимальный запас |JE энергии|. Ячейки вставляются в 4 гнезда на лицевой стороне (|ПКМ| ячейкой) и извлекаются |отвёрткой|.");
        add("tooltip.trd.machine.switch.desc", "|Рубильник| - специальный механизм, способный замыкать и размыкать |энергосеть| по нажатию |ПКМ| или при подаче редстоун-сигнала.");
        add("tooltip.trd.machine.converter.desc", "|Конвертер энергии| - преобразует |JE энергию| в стандартную |Forge энергию| и обратно. Регулировать пропуск |JE энергии| можно |Отвёрткой|.");
        add("tooltip.trd.machine.paintable_wire.desc", "|Окрашиваемый провод| - провод |энергосети|, умеющий маскироваться под другие блоки. Нажатие |ПКМ| любым блоком перекрашивает провод в его вид, при этом не расходуя сам блок.");
        add("tooltip.trd.machine.energy_cell.desc", "|Энергоячейка| - съёмный накопитель |модульного энергохранилища|. Вставляется в гнёзда на лицевой стороне корпуса, извлекается |отвёрткой|, сохраняя заряд в себе. Ёмкость и скорость передачи энергии складываются из значений всех вставленных ячеек.");
        ...
        add("tooltip.trd.machine.gear.desc", "|Шестерня| - насадка на вал для боковой передачи |крутящего момента|. Зацепляется с шестернями соседних валов, меняя направление вращения и задавая передаточное отношение. Снимается |отвёрткой|.");
        add("tooltip.trd.machine.bevel_gear.desc", "|Коническая шестерня| - насадка на торец вала для передачи |крутящего момента| под прямым углом. Соединяет перпендикулярные валы, чьи конические точки находятся рядом, с отношением 1 к 1. Снимается |отвёрткой|.");

        // ═══ Промышленные машины ═══
        add("tooltip.trd.machine.electro_furnace.desc", "|Электропечь| - печь работающая от |JE энергии|. Энергия принимается с задней стороны, расход - 100 JE в секунду. Плавит те же рецепты, что и ванильная печь, но на 30% быстрее, при этом накапливая опыт.");
        add("tooltip.trd.machine.stanok.desc", "|Мульти-заготовочный станок| - универсальная кинетическая машина для обработки материалов. Для работы требует непрерывной подачи |крутящего момента| через кинетические порты по бокам. Насадка в специальном слоте определяет тип операций. Каждый рецепт требует своих оборотов, при отклонении более чем на 25% работа останавливается.");
        add("tooltip.trd.machine.coccer_oven.desc", "|Коксовая печь| - специальная печь для высокотемпературной переработки. Скорость переработки растёт с превышением требуемой температуры. Не рекомендуется залезать в печь во время её работы.");
        add("tooltip.trd.machine.vishelashivatel.desc", "|рудный выщелачиватель| - кинетическая машина для обработки предметов путём взаимодействия с жидкостями. Тип рабочей жидкости задаётся |жидкостным идентификатором| в специальном слоте.");
        add("tooltip.trd.machine.centrifuge.desc", "|Центрифуга| - электрическая машина для разделения материалов на фракции. Собирается из |мотора центрифуги| и |специальных насадок|.");
        add("tooltip.trd.machine.steel_storage.desc", "|Стальное хранилище| - переносной склад-мультиблок на 91 слот. Не принимает шалкеры и другие переносные хранилища.");
        add("tooltip.trd.machine.casting_pot.desc", "|Литейный котёл| - часть цепочки литья, необходим для отлива деталей через |литейные формы|. Набрав необходимое количество жидкого металла, котёл создаёт раскалённую заготовку, которую нельзя брать рукой до её полного остывания. При долгом простое металл превращается в |шлак|.");
        add("tooltip.trd.machine.casting_descent.desc", "|Литейный желоб| - часть цепочки литья, передающая жидкий металл из |плавильни| позади себя в |литейные котлы|.");

        // ═══ Жидкости ═══
        add("tooltip.trd.machine.fuel_tank.desc", "|Цистерна| - многоблочная ёмкость для жидкостей, не подверженная коррозии и жару. Тип жидкости задаётся |жидкостным идентификатором|. Сохраняет жидкость при добыче.");
        add("tooltip.trd.machine.fluid_barrel.desc", "|Жидкостная бочка| - базовая ёмкость для жидкостей. Бочки чувствительны к содержимому: слишком горячая жидкость расплавит бочку, а коррозивная превратит в решето. |Протектор| в специальном слоте повышает стойкость к жару и коррозии.");
        add("tooltip.trd.machine.valve.desc", "|Жидкостный клапан| - элемент |жидкостной сети|, способный остановить передачу жидкостей нажатием |ПКМ| или при подаче |редстоун-сигнала|. Тип пропускаемой жидкости задаётся |Жидкостным идентификатором|.");
        add("tooltip.trd.machine.protector.desc", "|Протектор| - сменная защита для |жидкостной бочки|, вставляется в соответствующий слот. Повышает стойкость бочки к жару и коррозии.");

        // ═══ Инструменты ═══
        add("tooltip.trd.machine.screwdriver.desc", "|Отвёртка| - поистине самый универсальный инструмент в нашем моде. Позволяет настроить массу машин и установок. (P.S. Если вы нашли какой то баг, перед тем как писать нам о нём, попробуйте потыкать отвёрткой, авось все заработает).");
        add("tooltip.trd.machine.hammer.desc", "|Молот| - специальный инструмент для ковки на наковальне. Используется для ударов по заготовке при ручной ковке.");
        add("tooltip.trd.machine.poker.desc", "|Кочерга| - специальный инструмент для работы с горячим металлом. Позволяет достать из |литейного котла| раскалённую заготовку или |шлак|, не дожидаясь остывания. Нажатие |Shift + ПКМ| по |литейному котлу| сбрасывает все его содержимое в |шлак|.");
...
// ═══ ТУЛТИПЫ АККУМУЛЯТОРОВ (ModBatteryItem) ═══
        ...
        add(ModItems.HEATER_ITEM.get(), "Твердотопливный нагреватель");
