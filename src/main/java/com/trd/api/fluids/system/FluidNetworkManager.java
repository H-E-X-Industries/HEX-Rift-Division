...
public class FluidNetworkManager extends SavedData {
    ...
    public static FluidNetworkManager get(ServerLevel level) {
        return level.getDataStorage().computeIfAbsent(new net.minecraft.world.level.saveddata.SavedData.Factory<>(
                () -> new FluidNetworkManager(level), 
                (nbt, prov) -> new FluidNetworkManager(level, nbt), 
                null), 
                DATA_NAME
        );
    }
    ...
    @Override
    public @NotNull CompoundTag save(@NotNull CompoundTag nbt, net.minecraft.core.HolderLookup.Provider provider) {
