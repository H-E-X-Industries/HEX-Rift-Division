...
public class EnergyNetworkManager extends SavedData {
    ...
    private void addNodeInternal(BlockPos pos, @Nullable EnergyNetwork networkToAvoid,
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (neighbor == null && level.isLoaded(neighborPos)) {
                boolean isEnergyBlock = level.getCapability(ModCapabilities.ENERGY_PROVIDER, neighborPos, null) != null ||
                        level.getCapability(ModCapabilities.ENERGY_RECEIVER, neighborPos, null) != null ||
                        level.getCapability(ModCapabilities.ENERGY_CONNECTOR, neighborPos, null) != null;
    ...
    public boolean canConnectElectrically(BlockPos pos1, BlockPos pos2, Direction dir) {
        ...
        boolean can1 = checkConnection(pos1, dir);
        boolean can2 = checkConnection(pos2, dir.getOpposite());

        return can1 && can2;
    }

    private boolean checkConnection(BlockPos pos, Direction side) {
        // Проверяем Connector
        IEnergyConnector connCap = level.getCapability(ModCapabilities.ENERGY_CONNECTOR, pos, side);
        if (connCap != null) {
            return connCap.canConnectEnergy(side);
        }

        // Проверяем Provider
        IEnergyProvider provCap = level.getCapability(ModCapabilities.ENERGY_PROVIDER, pos, side);
        if (provCap != null) {
            return provCap.canConnectEnergy(side);
        }

        // Проверяем Receiver
        IEnergyReceiver recCap = level.getCapability(ModCapabilities.ENERGY_RECEIVER, pos, side);
        if (recCap != null) {
            return recCap.canConnectEnergy(side);
        }
