...
public class SpecialVeinFeature extends Feature<SpecialVeinConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<SpecialVeinConfiguration> context) {
        ...
        CrossChunkVeins.forEachVein(level, chunkX, chunkZ, params, vein -> {
            ...
            for (int x = x0; x <= x1; x++) {
                for (int z = z0; z <= z1; z++) {
                    for (int y = y0; y <= y1; y++) {
                        ...
                        // В 1.21.1 запрещено читать блоки за пределами генерируемого чанка.
                        // Пропускаем всё, что выходит за его границы.
                        if (x < minBX || x > maxBX || z < minBZ || z > maxBZ) continue;
            ...
            // Используем биом внутри ТЕКУЩЕГО чанка (центр жилы может быть в другом чанке —
            // обращаться к нему запрещено в 1.21.1). Состав жилы всё равно детерминирован
            // сидом жилы, поэтому разные чанки получают одинаковый composition.
            int safeBiomeX = net.minecraft.util.Mth.clamp(vein.cx, minBX, maxBX);
            int safeBiomeZ = net.minecraft.util.Mth.clamp(vein.cz, minBZ, maxBZ);
            VeinModifier modifier = VeinBiomeResolver.of(level.getBiome(new BlockPos(safeBiomeX, vein.cy, safeBiomeZ)));
