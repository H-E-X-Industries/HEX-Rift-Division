...
public class ConglomerateItem extends Item {
    ...
    public static void setAnalyzed(ItemStack stack) {
        CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> tag.putBoolean("Analyzed", true));
    }

    public static Map<FractionType, Integer> getFractions(ItemStack stack) {
        if (!stack.has(DataComponents.CUSTOM_DATA)) return Collections.emptyMap();
        CompoundTag tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    ...
    public static int getOU(ItemStack stack) {
        ...
        return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getInt("OU");
    }

    public static String getVeinType(ItemStack stack) {
        if (!stack.has(DataComponents.CUSTOM_DATA)) return "unknown";
        return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getString("VeinType");
    }
    ...
    public static float getFractionBoost(ItemStack stack, FractionType fraction) {
        if (!stack.has(DataComponents.CUSTOM_DATA)) return 1.0f;
        CompoundTag tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    ...
    private MutableComponent metalWithBoost(String metalId, float boost) {
        MutableComponent name = Component.literal(metalId)
                .withStyle(style -> style.withColor(metalDisplayColor(metalId, 0x767676)));
        if (boost != 1.0f) {
